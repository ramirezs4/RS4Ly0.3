from RS4LyAnálisisDeCondiciones02 import Puntuacion_GENERAL,Info_Geografica,Evaluar_Ultimo_Reporte,Trafico_NoHumano,Conexion_Proxy,Nodo_Tor,Limpiao_No1,Limpia_No2,Limpia_No3,Informacion_Resolucion,Urls_Detectadas,Puntuacion_IndividualJunta,Num_De_UsuariosYReportes
...
from RS4LyExtra02 import LimpiarPantalla,CM,FR,CG,CA,init,SB,SR,FC,BC,BF
init()
...
def Conjunto_Analisis_De_IP(IP1,Data1,Data2,Data3,Data4):
    ...
    PuntuacionIPabusedb=Puntuacion_IndividualJunta(Usar=Data1["data"]['abuseConfidenceScore'])
    PuntuacionIPqualityscore=Puntuacion_IndividualJunta(Usar=Data2["fraud_score"])
    PuntuacionIPvirustotal=Puntuacion_IndividualJunta(Usar=len(Data3["detected_urls"]))
    ...
    if AS=="1":
        LimpiarPantalla()
        TodoOk=Puntuacion_GENERAL(Data1,Data2)
        if TodoOk=="0":
            print(
                CA+"El analisis de la direccion "+IP1+" arrojo los siguientes resultados:"+FR
...
def Analisis_De_IP_Completo(IP1,Data1,Data2,Data3,Data4,Puntuaciongeneral,PuntuacionIPabusedb,PuntuacionIPqualityscore,PuntuacionIPvirustotal):
    LimpiarPantalla()
    NumeroUsuarios=Num_De_UsuariosYReportes(Data1=Data1,Para="usuario")
    NumeroReportes=Num_De_UsuariosYReportes(Data1=Data1,Para="reporte")
    ...
    input(BC+"\nPresione Enter para continuar"+BF)